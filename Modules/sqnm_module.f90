...
MODULE sqnm_module
   ...
   !! and cant be larger than the previous step multiplied by a certain 
   ...
   ! ... public methods
   !
   PUBLIC :: sqnm, init_sqnm, terminate_sqnm, sqnm_get_n_iter  
   !
...
CONTAINS
   !
   !------------------------------------------------------------------------
   SUBROUTINE init_sqnm( stdout_, bfgs_ndim_, trust_radius_max_, &
     ...
     !
   END SUBROUTINE init_sqnm
   !
   !------------------------------------------------------------------------
   SUBROUTINE sqnm( filebfgs, pos_in, h, nelec, energy, &
      ...
      !
      CALL read_sqnm_file( pos, grad, energy, n, lfcp, fcp_hess )
      !
      scf_iter = scf_iter + 1
      istep    = scf_iter
      !
...
#undef DEBUG
      ...
      IF ( .NOT. energy_wolfe_condition( energy ) .AND. (scf_iter > 1) .AND. (.NOT. lfcp) ) THEN
         ...
         IF ( trust_radius < trust_radius_min ) THEN
            ...
            END IF
            !
            CALL reset_sqnm( n, lfcp, fcp_hess )
            !
      ...
      !
      CALL write_sqnm_file( pos, energy, grad )
      !
...
1000  stop_bfgs = conv_bfgs
   ...
   !
   !------------------------------------------------------------------------
   SUBROUTINE reset_sqnm( n, lfcp, fcp_hess )
      !------------------------------------------------------------------------
      ...
      gdiis_iter = 0
      !
   END SUBROUTINE reset_sqnm
   !
   !------------------------------------------------------------------------
   SUBROUTINE read_sqnm_file( pos, grad, energy, n, lfcp, fcp_hess )
      !------------------------------------------------------------------------
      !
      IMPLICIT NONE
      !
      ...
      END IF
      !
   END SUBROUTINE read_sqnm_file
   !
   !------------------------------------------------------------------------
   SUBROUTINE write_sqnm_file( pos, energy, grad )
      !------------------------------------------------------------------------
      !
      IMPLICIT NONE
      !
      ...
      CLOSE( UNIT = iunbfgs )
      !
   END SUBROUTINE write_sqnm_file
   !
...
!               Conventional damping
   ...
   !
   !------------------------------------------------------------------------
   SUBROUTINE terminate_sqnm( energy, energy_thr, grad_thr, cell_thr, fcp_thr, &
      ...
      END IF
      !
   END SUBROUTINE terminate_sqnm
   !
   FUNCTION sqnm_get_n_iter (what)  RESULT(n_iter)
   !  
   IMPLICIT NONE
   INTEGER                         :: n_iter
   ...
   SELECT CASE (TRIM(what)) 
      ...
      CASE default 
           n_iter = -1
   END SELECT
   END FUNCTION sqnm_get_n_iter
END MODULE sqnm_module
